...
from .progress_util import ProgressUtils
...
def convert_member_json_to_array_startup(reg_no, team_name, startup_name, startup_email, startup_dor, startup_domain,
                                         startup_desc, member):
...
def update_google_sheets(type, reg_no, team_data, progress_id):
    ...
    if type == INDIVIDUAL:
        ...
        for member_no in members:
            member_converted = convert_member_json_to_array_individual(reg_no, team_name, team_email,
                                                                       members[member_no])
        ...
        for member_no in members:
            member_converted = convert_member_json_to_array_startup(reg_no, team_name, startup_name, startup_email,
                                                                    startup_dor, startup_domain, startup_desc,
                                                                    members[member_no])
            print(member_converted)
            body['values'].append(member_converted)
        ...
        for char in 'ABCDEFG':
            range_to_merge = re.sub(r'([A-Z])(\d)', repl=f'{char}\\2',
                                    string=updated_range)  # Convert A2:D2 to CHAR2:CHAR2
            ranges_to_merge.append(range_to_merge)

        merge_columns(ranges_to_merge)
...
def merge_columns(ranges_to_merge):
    service = get_spreadsheet_service()

    # Range Example: Individuals!A4:A5
    print("Range got: ".format(ranges_to_merge))
    ...
    for range_to_merge in ranges_to_merge:
        grid_range = get_grid_from_range(range_to_merge)
...
def get_grid_from_range(range_to_merge):
